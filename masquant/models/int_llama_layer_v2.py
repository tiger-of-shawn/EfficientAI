...
class QuantLlamaMLPV2(nn.Module):
    def __init__(
        self,
...
class QuantLlamaAttentionV2(nn.Module):
...
class QuantLlamaDecoderLayerV2(nn.Module):
    def __init__(self, 
                 config: LlamaConfig,
                 ori_layer,
                 args):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = QuantLlamaAttentionV2(
            org_module=ori_layer.self_attn,
            config=config,
            args=args,
            )
        self.mlp = QuantLlamaMLPV2(
