...
def smooth_ln_fcs_temporary(ln, fcs, scales,shifts):
    ...
    if isinstance(ln, QuantLinear):
        ln.temp_weight = ln.weight.T / scales
        ln.temp_weight = ln.temp_weight.T
    else:
...
def smooth_fc_fc_temporary(fc1, fc2, scales,shifts=None):
    ...
    if True or fc1.temp_weight.shape[0] != scales.view(-1,1).shape[0]:
...
def smooth_ln_fcs_inplace(ln, fcs, scales,shifts):
    ...
    if isinstance(ln, QuantLinear):
        temp_weight = ln.weight.T / scales
        ln.weight = temp_weight.T
    else:
...
def smooth_fc_fc_inplace(fc1, fc2, scales,shifts=None):
    ...
    return
    if fc1.weight.shape[0] != scales.view(-1,1).shape[0]:
        return    
...
def smooth_q_k_inplace(q_proj, k_proj, scales,):
    ...
    if k_proj.weight.shape[0] == scales.view(-1,1).shape[0]:
        k_proj.weight.mul_(scales.view(-1,1))
    if k_proj.bias.shape[0] == scales.view(-1).shape[0]:
