...
class LMMClass(lmms):
    def __init__(self, model_path, quant_model = None):
        ...
        if 'Qwen2.5-Omni' in model_path:
            ...
            import copy
            kwargs = {"device_map": 'auto', 'attn_implementation': 'flash_attention_2'}
            if quant_model is None:
                self._model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_path, **kwargs, torch_dtype=torch.float16)
            else:
                self._model = quant_model
            self.model = self._model.to('cuda')
            self._config = self._model.config
    ...
    def generate_until(self, requests: List[Instance]) -> List[str]:
        ...
        for chunk in chunks:
            ...
            # if inputs['input_ids'].device != self.model.model.layers[0].self_attn.q_proj.weight.device:
            #     self.model.to(inputs['input_ids'].device)
    ...
    def generate_until_bak(self, requests: List[Instance]) -> List[str]:
        res = []
